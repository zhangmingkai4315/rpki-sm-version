...
type TestManagerWithSM struct {
	FSManager    *TestingFileSeeker
	PrivateKeys  []*sm2.PrivateKey
...
func AddROAToManifestWithSM(manager *TestManagerWithSM, cmsBytes []byte) {
	...
	manager.FSManager.AddFile("rsync://lambda/module/certs/test.roa", cmsBytes)
	...
	certBytesMft, _ := sm2.CreateCertificate(
		...
		manager.PrivateKeys[SUBCERT_KEY_INDEX])
	...
	manifestCms.SignSM(rand.Reader,
		...
		encap, manager.PrivateKeys[MANIFEST_KEY_INDEX_2], certBytesMft)

	cmsManifestBytes, _ := asn1.Marshal(*manifestCms)

	manager.FSManager.AddFile("rsync://lambda/module/certs/test.mft", cmsManifestBytes)
}
...
func SignROAWithSM(manager *TestManagerWithSM) ([]byte, error) {
	...
	certBytesRoa, _ := sm2.CreateCertificate(
		rand.Reader, roaCert, manager.OrgCert, manager.PublicKeys[ROA_KEY_INDEX], manager.PrivateKeys[SUBCERT_KEY_INDEX])

	encap, _ := librpki.ROAToEncap(roaContentEnc)

	_ = roaCms.SignSM(rand.Reader,
		manager.skiPublicKey[ROA_KEY_INDEX],
		encap, manager.PrivateKeys[ROA_KEY_INDEX],
		certBytesRoa)
